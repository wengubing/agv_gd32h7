...
//外部方法声明
extern void CanFestivalTimer_Config();
extern void CanFestivalTimer_Enable(ControlStatus NewState);
...
//外部对象
extern u32 g_Sys_1ms_Counter;

//内部定义对象声明
UNS32 InitDataBuffer=0;
SVO_DATA_TypeDef 	SVO[SVO_MAXNUM];	//伺服对象
...
{
	u32	i;
	CAN_TX_FIFO	*pCan_tx_Buff = (CAN_TX_FIFO*)&(UNI_CAN[useCAN1].tx_fifo);
	
	if(pCan_tx_Buff && m->len < 9)
	{
		pCan_tx_Buff->can_Tx_msg[pCan_tx_Buff->write_adr].StdId = m->cob_id;
		if(m->rtr)
			pCan_tx_Buff->can_Tx_msg[pCan_tx_Buff->write_adr].RTR = CAN_RTR_REMOTE;//远程帧
		else
			pCan_tx_Buff->can_Tx_msg[pCan_tx_Buff->write_adr].RTR = CAN_RTR_DATA;//数据帧
		pCan_tx_Buff->can_Tx_msg[pCan_tx_Buff->write_adr].IDE = CAN_ID_STD;
		pCan_tx_Buff->can_Tx_msg[pCan_tx_Buff->write_adr].DLC = m->len;
		for(i = 0; i < m->len; i++)
		{
			pCan_tx_Buff->can_Tx_msg[pCan_tx_Buff->write_adr].Data[i] = m->data[i];
		}

		if(++ pCan_tx_Buff->write_adr >= CAN_RX_BUF_LENGTH_MAX)		
			pCan_tx_Buff->write_adr = 0;
	}
		...
		if(CanMaster.SlaveParaConfigId[i] == i+1)
		{
			if((g_ucDriverMode[i]==DRIVER_MODE_VELOCITY) || (g_ucDriverMode[i]==DRIVER_MODE_ImmediateVelocity))   //速度模式 || 立即速度模式	
			{
				...
				else if(CanMaster.Para.SVOType == Can_SVOType_Delta)
				{
						...
						{
							CanObjectDict_Tx_Map12[i] = 90;//g_Para.Walk.TorqueLimit_Stop;
						}
						else
						{
...
/**/
void canfestival_timer_start()
{

	CanFestivalTimer_Config();//配置定时器
...
//============================================================================
/**/
void can_master_init(UNI_CANi *p_stCAN, CO_Data* d, ControlStatus NewState)
{
	if(p_stCAN->ini.Baud <= CAN_BAUDRATE_1M && p_stCAN->ini.Baud >= CAN_BAUDRATE_5K)
	{
		can_od_init(d);
	}
	
	CanFestivalTimer_Enable(NewState);//仅使能操作,定时器配置在"bsp.c"中完成
...
*	功能说明: canopen接收数据包处理
*********************************************************************************************************/
void Tsk_CanOpen_TxMsgPro(CO_Data* d, UNI_CANi *p_stCAN)
{
		can_send_msg(p_stCAN->ini.CANx, p_stCAN->tx_fifo.can_Tx_msg[p_stCAN->tx_fifo.read_adr]);
		if(++ p_stCAN->tx_fifo.read_adr >= CAN_RX_BUF_LENGTH_MAX)
		{
			p_stCAN->tx_fifo.read_adr = 0;		
		}
}
...
*	功能说明: canopen接收数据包处理
*********************************************************************************************************/
void Tsk_CanOpen_RxMsgPro(CO_Data* d, UNI_CANi *p_stCAN)
{
		Message Msg, *pMsg;

		pMsg = &Msg;

		pMsg->len = p_stCAN->rx_fifo.can_rx_msg[p_stCAN->rx_fifo.read_adr].DLC;
		if(pMsg->len)
		{
			pMsg->cob_id = p_stCAN->rx_fifo.can_rx_msg[p_stCAN->rx_fifo.read_adr].StdId;			
			pMsg->rtr = p_stCAN->rx_fifo.can_rx_msg[p_stCAN->rx_fifo.read_adr].RTR;			
			memcpy(pMsg->data, p_stCAN->rx_fifo.can_rx_msg[p_stCAN->rx_fifo.read_adr].Data, pMsg->len);
			CanFestivalTimer_Ctrl(DISABLE);
			
//				SlaveHeartMessageProcess(d, pMsg);				
		...
		if(++ p_stCAN->rx_fifo.read_adr >= CAN_RX_BUF_LENGTH_MAX)
		{
			p_stCAN->rx_fifo.read_adr = 0;		
		}
}
...
//canopen从站配置 初始化任务
bool LinkCO_ParaIni(LinkCanOpen *pCm, CO_Data* d, UNI_CANi *p_stCAN)
{
	p_stCAN->rx_fifo.read_adr=0;
	p_stCAN->rx_fifo.write_adr=0;
	p_stCAN->tx_fifo.read_adr=0;
	p_stCAN->tx_fifo.write_adr=0;
	...
	NMT_Reset_NodeIDCnt = 0;//上线节点数强制改为0；canopen协议栈软件重启时需要置0
...
//canopen从站配置 伺服下电
bool LinkCO_DrivePowerDown(LinkCanOpen *pCm, CO_Data* d, UNI_CANi *p_stCAN)
{
	u8 i=0;
	//断电操作待添加
	if(p_stCAN->CanStatusFlag==CAN_NoneError)
	{
		can_master_init(p_stCAN, d, ENABLE);
		setState(d, Stopped);
	...
	{
		can_master_init(p_stCAN, d, DISABLE);
		p_stCAN->CanStatusFlag = CAN_NoneError;//清除错误标志
		CanMaster.MasterStatu = Disconnected;
		for(i=0;i<MAX_NodeId_NUM;i++)
		{
			SVO[i].SlaveStatus=Disconnected;
		}
...
//canopen从站配置 伺服上电
bool LinkCO_DrivePowerUp(LinkCanOpen *pCm, CO_Data* d, UNI_CANi *p_stCAN)
{
	if(Delay_Fuction(g_Sys_1ms_Counter, canopen_delay_cnt, 500))//等待
	{
		//上电操作待添加
...
//canopen从站配置 复位节点
bool LinkCO_ResetNode(LinkCanOpen *pCm, CO_Data* d, UNI_CANi *p_stCAN)
{
	if(Delay_Fuction(g_Sys_1ms_Counter, canopen_delay_cnt, 3000))
	{//等待3秒,上电完成,
		masterSendNMTstateChange(d, 0x00, NMT_Stop_Node);//节点停止状态(PDO通讯停止,可进行SDO和NMT)
		masterSendNMTstateChange(d, 0x00, NMT_Reset_Node);//复位node应用层
...
//canopen从站配置 复位通讯
bool LinkCO_ResetComunication(LinkCanOpen *pCm, CO_Data* d, UNI_CANi *p_stCAN)
{
//	if(Delay_Fuction(g_Sys_1ms_Counter, canopen_delay_cnt, 500))
//	{
...
//canopen从站配置 复位通讯
bool LinkCO_WaitBootUpOk(LinkCanOpen *pCm, CO_Data* d, UNI_CANi *p_stCAN)
{
	if(Delay_Fuction(g_Sys_1ms_Counter, canopen_delay_cnt, 500))
	{//4秒内，固定时间
		u8 i,id;
...
//canopen循环任务
void tsk_canopen_master(LinkCanOpen *pCm, CO_Data* d, UNI_CANi *p_stCAN)
{
	//u8 i=0;
	//static u32 canopen_delay_cnt;

	if(p_stCAN->CanStatusFlag==CAN_TransmitError)
	{//在非静止canopen_master的情形下,出现主机链路层异常 或 断线  !!!
		p_stCAN->CanStatusFlag=CAN_GoToFault;
