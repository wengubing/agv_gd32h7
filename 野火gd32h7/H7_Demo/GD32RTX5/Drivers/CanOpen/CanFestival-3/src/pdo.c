...
UNS8 buildPDO (CO_Data * d, UNS8 numPdo, Message * pdo)
{
  ...
  UNS8 prp_j = 0x00;
  UNS8 mappingCount = 0x00;
  UNS8 mappingCountLimit = 0x00;
  ...
  d->SlaveNodeid = (pdo->cob_id & 0x7F);//用于定位PDO_TRS,LinkMaster_Tx_Map10/LinkMaster_Tx_Map11映射到个Slave的index编号(_getODentry()中使用)，
																				//新增...2016.01.06---邓洋
    
  if(d->SlaveNodeid > d->CurrntMaxNodeid)
  {
    return 0xFF;                        //用于解决PDO发送产生一包无效数据     新增...2018.03.19---by biyabc
  }
  
  if (TPDO_map->bSubCount > 0)
  {
    mappingCountLimit = (UNS8)(TPDO_map->bSubCount - 1);
  }

  mappingCount = *pMappingCount;
  if (mappingCount > mappingCountLimit)
  {
    MSG_ERR (0x1014, " PDO map count exceeds subindex count : ", mappingCount);
    mappingCount = mappingCountLimit;
  }
  ...
  do
    {
      ...
      if ((prp_j + 1U) >= TPDO_map->bSubCount)
      {
        MSG_ERR (0x1015, " PDO map subindex out of range : ", prp_j + 1U);
        return 0xFF;
      }
                ...
                while (numMap < *pMappingCount)
                  {
                    UNS8 tmp[] = { 0, 0, 0, 0, 0, 0, 0, 0 };
										UNS32 ByteSize;
										d->SlaveNodeid = (UNS8)GET_NODE_ID((*m));//用于Callback_Rx_Mapx,判断是那个从机的数据,update_2015.12.31,by邓洋
