...
//外部方法声明
extern void CanFestivalTimer_Config(TIM_TypeDef *Timx, UNS32 Rcc, UNS8 NvicIrq, INTEGER8 pi, FunctionalState NewState);
...
{
	u32	i;
	CAN_TX_FIFO	*pCan_tx_Buff = (CAN_TX_FIFO*)&(STM32_CAN[useCAN1].tx_fifo);
...
void canfestival_timer_start(u8 pi,  FunctionalState NewState)
{
	...
	CanFestivalTimer_Config(CanFestivalTimer_Set, pi, NewState);//配置定时器
...
//canopen从站配置 初始化任务
bool LinkCO_ParaIni(LinkCanOpen *pCm, CO_Data* d, STM32_CANi *p_stCAN)
{
	p_stCAN->rx_fifo.read_adr=0;
...
//canopen从站配置 伺服下电
bool LinkCO_DrivePowerDown(LinkCanOpen *pCm, CO_Data* d, STM32_CANi *p_stCAN)
{
	u8 i=0;
	//断电操作待添加
...
//canopen从站配置 伺服上电
bool LinkCO_DrivePowerUp(LinkCanOpen *pCm, CO_Data* d, STM32_CANi *p_stCAN)
{
...
//canopen从站配置 复位节点
bool LinkCO_ResetNode(LinkCanOpen *pCm, CO_Data* d, STM32_CANi *p_stCAN)
{
...
//canopen从站配置 复位通讯
bool LinkCO_ResetComunication(LinkCanOpen *pCm, CO_Data* d, STM32_CANi *p_stCAN)
{
//	if(Delay_Fuction(g_Sys_1ms_Counter, canopen_delay_cnt, 500))
//	{
...
//canopen从站配置 复位通讯
bool LinkCO_WaitBootUpOk(LinkCanOpen *pCm, CO_Data* d, STM32_CANi *p_stCAN)
{
	if(Delay_Fuction(g_Sys_1ms_Counter, canopen_delay_cnt, 500))
	{//4秒内，固定时间
		u8 i,id;
