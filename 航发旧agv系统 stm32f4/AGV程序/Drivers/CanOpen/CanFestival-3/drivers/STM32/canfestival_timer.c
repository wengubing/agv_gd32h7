...
static TIMEVAL last_time_set = 0; // 之前初始化为 TIMEVAL_MAX，会导致首次计算异常，改为 0 并在初始化时同步
...
TIMEVAL getElapsedTime(void)
{
	...
	// 正确处理计数器回绕（counter 为无符号 16 位）
	if (timer >= last) {
		return (TIMEVAL)(timer - last);
	} else {
		// 回绕时，elapsed = (ARR - last) + timer + 1
...
//		TIM_ARRPreloadConfig(Timx, DISABLE);// Timx->ARR,禁止ARR自动重装载缓冲器
    ...
    /*定时器中断向量配���*/ 
		...
		// 在定时器启动前同步 last_time_set，避免首次 getElapsedTime 产生错误值
