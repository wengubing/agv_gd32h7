...
volatile uint8_t  usbd_msc0_media_own1;
volatile uint8_t  usbd_msc0_media_own1;  // USB MSC0 media ownership
static   int32_t  drv_id1;               // FAT drive id
static   bool     media_ok1;             // Media is initialized and ok
...
void USBD_MSC0_Initialize (void) {
  ...
  /////////////////////////////////////////////////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////////////////////////  
  uint32_t param_status1;
 
  usbd_msc0_media_own1 = MEDIA_OWN_USB;  // Initially media is owned by USB
  media_ok1            = false;          // Current media status (not initialized = not ok)
 
  if (finit (MEDIA_DRIVE1) != fsOK) {    // Initialize File System
    return;                              // Exit if failed
  }
 
  drv_id1 = fs_ioc_get_id (MEDIA_DRIVE1); // Get ID of media drive
  if (drv_id1 < 0)           { return; }  // If ID is invalid exit
 
  param_status1 = 0U;                    // Parameter for function call is 0
                                         // Initialize media
  if (fs_ioc_device_ctrl (drv_id, fsDevCtrlCodeControlMedia, &param_status1) != fsOK) {
    return;                              // Exit if failed
  }
 
  if (fs_ioc_lock (drv_id1)) {           // Lock media for USB usage
    return;                              // Exit if failed
  }
 
  media_ok1 = true;                      // Media was initialized and is ok
#else
...
bool USBD_MSC0_LUN_GetMediaCapacity (uint8_t lun, uint32_t *block_count, uint32_t *block_size) {
  ...
  {
      // Read media information of actual media
      if (fs_ioc_read_info(drv_id1, &media_info) != fsOK) {
        return false;                       // Exit if failed
      }
...
bool USBD_MSC0_LUN_Read (uint8_t lun, uint32_t lba, uint32_t cnt, uint8_t *buf) {
...
#if (USE_FILE_SYSTEM == 1)              // If File System is used
  if(lun == 0)                                     // Read data directly from media
  {
  ...
  {
     if (fs_ioc_read_sector (drv_id1, lba, buf, cnt) != fsOK) {
       return false;
     }
  }
#else
...
bool USBD_MSC0_LUN_Write (uint8_t lun, uint32_t lba, uint32_t cnt, const uint8_t *buf) {
  ...
  {
      if (fs_ioc_write_sector (drv_id1, lba, buf, cnt) != fsOK) {
        return false;
      }    
  }
#else
...
static uint8_t  media_ready_ex = 0U;    // Previous media ready state
  ...
  else
  {
     // Get current media status
      media_state = 0U;
      switch (fs_ioc_device_ctrl (drv_id1, fsDevCtrlCodeCheckMedia, &param_status)) {
        case fsOK:
          if (param_status & FS_MEDIA_NOCHKMEDIA) {
            // If check media not available on hardware layer
            media_state  =  USBD_MSC_MEDIA_READY;
      ...
      if (own & MEDIA_OWN_CHG) {                    // If owner change requested
        if (own & MEDIA_OWN_USB) {                  // If new requested owner is USB (previous owner was File System)
          ...
          (void)fs_ioc_unlock (drv_id1);             // Un-lock media
        }
      }
      ...
      if ((own & MEDIA_OWN_CHG)        ||           // If owner change requested or
        ...
        if (media_state & USBD_MSC_MEDIA_READY) {   // If media is ready
          if (own & MEDIA_OWN_USB){                 // If current owner is USB
            media_ok1     = false;                   // Invalidate current media status (not initialized = not ok)
            param_status = 0U;                      // Parameter for function call is 0
            if (fs_ioc_device_ctrl (drv_id1, fsDevCtrlCodeControlMedia, &param_status) == fsOK) {
                                                    // Initialization of media has succeeded
              if (fs_ioc_lock (drv_id1) == fsOK) {   // If lock media for USB usage has succeeded
                media_ok = true;                    // Media was initialized and is ok
              }
            }
          } else {                                  // If current owner is File System
            if (fmount (MEDIA_DRIVE1) == fsOK) {     // Initialize media and Mount Drive for File System usage
              media_ok = true;                      // Media was initialized and is ok
            }
          }
        }
        if (own & MEDIA_OWN_CHG) {
          usbd_msc0_media_own1 &= ~MEDIA_OWN_CHG;    // Clear request to change media owner if it was handled
        }
        media_ready_ex = media_state & USBD_MSC_MEDIA_READY;
      }
     
      // If media is not ok or owned by File System return that it is not ready for USB
      if ((!media_ok1) || (!(usbd_msc0_media_own1 & MEDIA_OWN_USB))) {
        return 0U;
      }    
  }
